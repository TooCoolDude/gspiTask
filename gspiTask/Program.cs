...
namespace gspiTask
{
    public class Program
    {
        ...
        {
            var similarSurname = list.Where(p => IsSimilarSurname(p.Surname, "Кузин"));
            return new DTO()
            {
                ...
                SimilarSurname = similarSurname.Count() > 0 ? similarSurname.First().Surname : string.Empty
            };
        }
        ...
        private static bool IsSimilarSurname(string s1, string s2)
        {
            ...
            foreach (var c in Enumerable.Range(1072, 1103))
            {
                for (int i = 0; i < s1.Length - 1; i++)
                {
                    s1variants.Add(s1.Substring(0, i) + (char)c + s1.Substring(i + 1, s1.Length - i - 1));
                }
                for (int i = 0; i < s2.Length - 1; i++)
                {
                    s2variants.Add(s2.Substring(0, i) + (char)c + s2.Substring(i + 1, s2.Length - i - 1));
                }
            }
